...
export class ExportService {
  async exportSales(startDate?: Date, endDate?: Date, saleType?: string) {
    ...
    for (const sale of sales) {
      worksheet.addRow({
        ...
        months: sale.saleType === 'INSTALLMENT' ? `${sale.months} شهر` : 'سداد كامل',
        saleType: sale.saleType === 'CASH' ? 'سداد كامل' : 'أقساط',
  ...
  async exportCollections(startDate?: Date, endDate?: Date) {
    ...
    for (const payment of payments) {
      worksheet.addRow({
        ...
        months: payment.sale.saleType === 'INSTALLMENT' ? `${payment.sale.months} شهر` : 'سداد كامل',
        paymentType: payment.paymentType === 'CASH_SALE' ? 'سداد كامل' : payment.paymentType === 'DOWN_PAYMENT' ? 'دفعة مقدمة' : 'قسط',
  ...
  async exportOverdue(startDate?: Date, endDate?: Date) {
    ...
    worksheet.columns = [
      ...
      { header: 'كود العميل', key: 'bkCode', width: 15 },
      { header: 'نوع العميل', key: 'customerType', width: 15 },
      { header: 'الإدارة', key: 'department', width: 15 },
      { header: 'رقم الماكينة', key: 'machineSerial', width: 20 },
      { header: 'نظام التقسيط', key: 'months', width: 12 },
    ...
    for (const inst of overdue) {
      worksheet.addRow({
        ...
        department: inst.sale.customer.department || '',
